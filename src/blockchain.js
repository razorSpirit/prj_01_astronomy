...
const GENESIS_BLOCK_DATA = {data: "Genesis Block"};
...
class Blockchain {
    ...
    async initializeChain() {
        if( this.height === -1){
            let block = new BlockClass.Block(GENESIS_BLOCK_DATA);
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let diffInSeconds = Math.floor((currentTime - msgTime) / 1000);

            if(diffInSeconds >= 300) {
                let test = bitcoinMessage.verify(message, address, signature);
                reject("Message is greater then 5 minutes.");
            }
